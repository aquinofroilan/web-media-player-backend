...
async function needsAudioTranscoding(filePath: string): Promise<boolean> {
  return new Promise((resolve) => {
    ffmpeg.ffprobe(filePath, (err, data) => {
      ...
      // Transcode if not AAC or has more than 2 channels
      const codecName = audioStream.codec_name ?? '';
      const channels = audioStream.channels ?? 2;
      const needsTranscode = codecName !== 'aac' || channels > 6;
...
export async function streamHandler(
  ...
  try {
    ...
    if (transcode) {
      ...
      await streamWithTranscode(filePath, res);
...
function streamWithRange(
  ...
  if (range === null) {
    res.status(416).set('Content-Range', `bytes */${String(fileSize)}`).end();
    return;
  }
 ...
 */
async function streamWithTranscode(filePath: string, res: Response): Promise<void> {
